...
interface DataBaseRepository : JdbcRepository {
    ...
    @Query("select name, game_data, kit, money, donate_group, arena, kills, current_kills, deaths, local_booster, active_booster, booster_time, position, inventory, potion_effects, available_kits, permissions FROM kitpvp.kit_players where name = :name")
    fun loadPlayerData(name: String): KitPlayer?

    @Query("update kitpvp.kit_players set name = :data.name, game_data = :data.gameData, kit = :data.kit, money = :data.money, donate_group = :data.donateGroup, arena = :data.arena, kills = :data.kills, current_kills = :data.currentKills, deaths = :data.deaths, local_booster = :data.localBooster, active_booster = :data.activeBooster, booster_time = :data.boosterTime, position = :data.position, inventory = :data.inventory, potion_effects = :data.potionEffects, available_kits = :data.availableKits, permissions = :data.permissions where name = :data.name")
    ...
    @Query("insert into kitpvp.kit_players (name, money, donate_group, arena, kills, deaths, current_kills, kit, local_booster, active_booster, booster_time, permissions) values (:data.name, :data.money, :data.donateGroup, :data.arena, :data.kills, :data.deaths, :data.currentKills, :data.kit, :data.localBooster, :data.activeBooster, :data.boosterTime, :data.permissions)")
